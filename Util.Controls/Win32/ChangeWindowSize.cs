...
namespace Util.Controls
{
    ...
    public class ChangeWindowSize
    {
        ...
        {
            // 获得窗体的 样式
            int oldstyle = NativeMethods.InvokeGetWindowLong(hwnd, NativeMethods.GWL_STYLE);
            if (msg == WMNCHITTEST)
            {
                ...
                // 更改窗体的样式为无边框窗体
                NativeMethods.InvokeSetWindowLong(hwnd, NativeMethods.GWL_STYLE, oldstyle & ~NativeMethods.WS_CAPTION);
            ...
            {
                // 在将要改变的时候，是样式添加系统菜单
                NativeMethods.InvokeSetWindowLong(hwnd, NativeMethods.GWL_STYLE, oldstyle & ~NativeMethods.WS_CAPTION | NativeMethods.WS_SYSMENU);
            }
            return IntPtr.Zero;
        }
    }
